...
function autoDetectCategory(team) {
  ...
  // P1.4 — Bugfix: `^men\b` matchte "Men2" nicht, weil \b zwischen n und 2
  // (beide \w) keine Wort-Grenze ist. `\bmen\d*\b` matcht "men", "men1", "men2".
  // Greift nicht auf "women" (kein \b vor m) und nicht auf "menschen"
  // (kein \b zwischen n und s).
  if (/herren|\bmen\d*\b|männer|mens/.test(txt))  return 'herren';
  return null;
}
...
export async function listTournaments() {
  ...
  try {
    ...
    if (index?.tournaments) {
      // Configs parallel laden — sequenziell war das ein N+1 auf dem
      // Landing-Page-Pfad (1 Read pro Turnier, bei 20 Turnieren spürbar).
      const configs = await Promise.all(
        index.tournaments.map(entry =>
          store.get(`${entry.slug}/config.json`, { type: 'json', consistency: 'strong' })
            .catch(() => null)
        )
      );
      for (const cfg of configs) {
        if (!cfg || cfg._deleted === true) continue;   // Tombstone überspringen
        found.set(cfg.slug, cfg);
      }
    }
