...
export async function listAutoEntriesForReferee(refereeId, year) {
  ...
  // N1 R3 — year=null/undefined → alle Jahre. Wird im Profil-Tab „Alle" genutzt,
  // damit Schiris auch historische Einsätze finden und Fehleingaben aufspüren.
...
export async function aggregateEntriesForReferee(refereeId, year) {
  ...
  // N2 — `m.spielklasse` ist der DKV-Code (herren/damen/junioren/jugend/schueler).
  // dkvPdf.mjs#SPIELKLASSE_TO_COLUMN konsumiert ihn direkt als `division`.
  // Legacy-Einträge ohne `spielklasse` bleiben mit '' → leere Spalte im PDF.
  const manualMapped = manual.map(m => ({
    date:           m.tournamentDate,
    matchNr:        m.matchNr || '',
    tournamentName: m.tournamentName,
    role:           m.role,
    division:       m.spielklasse || '',
    notes:          m.notes || '',
  }));
